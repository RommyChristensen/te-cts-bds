// The Logic Lock - Code Breaking Game
class GameInstance {
    constructor(options) {
    ...
    setupCanvas() {
        ...
        this.canvas.style.border = '2px solid #9C27B0';
        ...
        rows.forEach((row) => {
            ...
            row.forEach((buttonText) => {
                ...
                button.style.cssText = `
                    ...
                    border: 2px solid ${isEnter ? '#9C27B0' : isClear ? '#f44336' : '#9C27B0'};
                    border-radius: 8px;
                    cursor: pointer;
                ...
                button.addEventListener('mouseenter', () => {
                    ...
                    button.style.borderColor = isEnter ? '#BA68C8' : isClear ? '#ef5350' : '#BA68C8';
                });

                button.addEventListener('mouseleave', () => {
                    button.style.transform = 'translateY(0)';
                    button.style.boxShadow = '0 3px 6px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.2)';
                    button.style.borderColor = isEnter ? '#9C27B0' : isClear ? '#f44336' : '#9C27B0';
                });
    ...
    async loadQuestions() {
        try {
            const response = await fetch('/data/logic-lock-questions.json');
            const data = await response.json();
            this.logicLockQuestions = data;
        } catch (error) {
    ...
    async init() {
        console.log('Initializing The Logic Lock game...');
        ...
        this.currentQuestion = this.logicLockQuestions.find(q => {
    ...
    start(persistentStartTime = null) {
        console.log('Starting The Logic Lock game...');
        this.isRunning = true;
        ...
        console.log('Ending The Logic Lock game...');
        this.isRunning = false;
        ...
        const result = {
            ...
            correct: this.submittedAnswer === this.currentQuestion.answer.replace(/\s/g, ''),
    ...
    submitAnswer() {
        ...
        const correctAnswer = this.currentQuestion.answer.replace(/\s/g, '');
    ...
    drawBackgroundPattern() {
        this.ctx.fillStyle = 'rgba(156, 39, 176, 0.05)';
    ...
    drawWaitingScreen() {
        ...
        this.ctx.fillText('THE LOGIC LOCK', this.canvas.width / 2, 150);
        
        this.ctx.fillStyle = '#c0c0c0';
        this.ctx.font = '24px Arial';
        this.ctx.fillText('Code Breaking Challenge', this.canvas.width / 2, 200);
        ...
        this.ctx.fillText('Crack the 3-digit code using the clues', this.canvas.width / 2, 320);
    ...
    drawGameScreen() {
        ...
        const clueSize = this.isMobile ? 12 : 14;
        
        this.ctx.fillStyle = '#9C27B0';
        this.ctx.font = `bold ${titleSize}px Arial`;
        this.ctx.textAlign = 'center';
        const titleY = this.isMobile ? 40 : 60;
        this.ctx.fillText('CRACK THE CODE', this.canvas.width / 2, titleY);
        ...
        // Draw clues
        this.ctx.fillStyle = '#FFC107';
        this.ctx.font = `${clueSize}px monospace`;
        this.ctx.textAlign = 'left';
        
        let clueY = this.isMobile ? 100 : 130;
        const clueLineHeight = this.isMobile ? 18 : 22;
        
        this.ctx.fillText('Clues:', 40, clueY);
        clueY += clueLineHeight;
        
        if (this.currentQuestion && this.currentQuestion.clues) {
            this.currentQuestion.clues.forEach((clue, index) => {
                this.ctx.fillStyle = '#c0c0c0';
                this.ctx.font = `${clueSize}px monospace`;
                ...
                for (let word of words) {
                    ...
                    if (metrics.width > maxWidth && line) {
                        this.ctx.fillText(line, 50, clueY);
                        clueY += clueLineHeight * 0.8;
                        line = word;
                    } else {
                        line = testLine;
                    }
        ...
        if (this.isFinished) {
            const correctAnswer = this.currentQuestion.answer.replace(/\s/g, '');
            const isCorrect = this.submittedAnswer === correctAnswer;
    ...
    drawAnswerInput() {
        ...
        const boxY = this.isMobile ? this.canvas.height - 200 : this.canvas.height - 180;
        const digitSize = this.isMobile ? 28 : 36;
        
        this.ctx.strokeStyle = this.isFinished ? '#666' : '#9C27B0';
    ...
    drawFinishedScreen() {
        ...
        this.ctx.font = 'bold 48px Arial';
        this.ctx.textAlign = 'center';
        this.ctx.fillText(isCorrect ? 'CODE CRACKED!' : 'TRY AGAIN NEXT TIME', this.canvas.width / 2, 150);
        ...
        if (isCorrect) {
            this.ctx.fillText(`Code: ${correctAnswer}`, this.canvas.width / 2, 300);
    ...
    drawLockBorder() {
        ...
        this.ctx.fillStyle = '#9C27B0';
        this.ctx.font = '12px monospace';
        this.ctx.fillText(lockPattern, 20, 50);
        this.ctx.fillText(lockPattern, this.canvas.width - 100, this.canvas.height - 20);
    }
    ...
    cleanup() {
        ...
        console.log('The Logic Lock game cleaned up');
    }
}
